...
def setup_commands_routs(dp: Dispatcher) -> None:
    dp.register_message_handler(start_adding_balance, RoleFilter(["admin"]), commands="add_balance", state="*")


def setup_routs(dp: Dispatcher) -> None:
    dp.register_message_handler(select_user, RoleFilter(["admin"]), state=AddBalanceStates.select_user)
    ...
    dp.register_message_handler(input_amount, RoleFilter(["admin"]), state=AddBalanceStates.input_amount)
